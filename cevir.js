...
Asena.addCommand({pattern: 'cevir$', fromMe: true, desc: Lang.STİCKER_DESC}, (async (message, match) => {
        ...
        ffmpeg(location)
            ...
            .save('output.png')
            .on('end', async () => {
                await message.client.sendMessage(mid, fs.readFileSync('output.png'), MessageType.image, {mimetype: Mimetype.png, caption: 'YNH BURADA!'});
            });
        ...
        await webp2mp4File(savedFilename).then(async (rest) => {
            await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
                ...
                saving.on("finish", async () => {
                    await message.client.sendMessage(mid, fs.readFileSync('/root/WhatsAsenaDuplicated/stweb.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: 'YNH BURADA!', quoted: message.data })
